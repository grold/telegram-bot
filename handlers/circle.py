...
async def cmd_share(message: types.Message, command: CommandObject):
    """Handles the /share [on|off|update] command."""
    args = command.args
    ...
    if args == "on":
        ...
        if user_record and user_record['latitude'] is not None:
            await message.answer("Location sharing is now ON. I'll use your last known location. You can send a new location anytime to update it.")
        else:
            builder = ReplyKeyboardBuilder()
            builder.button(text="📍 Share Current Location", request_location=True)
            await message.answer(
                "Location sharing is now ON! To start appearing in the circle, please share your current location using the button below:",
                reply_markup=builder.as_markup(resize_keyboard=True, one_time_keyboard=True)
            )
    elif args == "off":
        update_user_status(user.id, user.username, user.full_name, False)
        await message.answer("Location sharing is now OFF. You won't be visible on the map and won't see others.")
    elif args == "update":
        ...
        await message.answer("Usage: <code>/share on</code>, <code>/share off</code>, or <code>/share update</code>")
...
async def cmd_map(message: types.Message, command: CommandObject):
    ...
    if not args or args == "list":
        ...
        if not sharing_users:
            await message.answer("No one is currently sharing their location in the circle.")
            return
            
        response = "<b>Circle of Friends:</b>\n\n"
        for u in sharing_users:
            name = u['username'] if u['username'] else u['full_name']
            response += f"• @{name} - /map_{name}\n"
        ...
        if not target or not target['is_sharing'] or target['latitude'] is None:
            await message.answer(f"Sorry, I couldn't find user @{target_name} in the sharing circle.")
            return
