...
- **Technical Mandate (CLI):** When adding new commands to the CLI, ensure that the `MockMessage` and other mock objects in `tools/mock_aiogram.py` are updated to include all attributes and methods used by the command's handler (e.g., `message.location`, `message.answer_photo`).
